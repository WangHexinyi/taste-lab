# V4.3 Spatial Experiment

This experimental build keeps the V4.2 portable architecture and changes the spatial/3D policy only. Its purpose is to test whether agents can:

- keep useful 2.5D mouse-linked depth on abstract interface modules;
- avoid fake large-angle 3D claims on flat physical-product representations;
- actively choose true 3D / hybrid spatial media when product form, material, configuration, or immersive experience benefits from it;
- treat 3D as a first-class medium rather than an automatically discouraged effect.

See `EXPERIMENT.md` and `references/spatial-media.md`.
