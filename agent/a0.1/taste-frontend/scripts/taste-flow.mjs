 ...
 * Zero npm dependencies. Run from the user's project/workspace root.
 *
 * Usage:
 ...
 *
 * The agent, not the human, is expected to call this when terminal execution is available.
 */
...
if(cmd==='init'){
  ...
  const state = {
    ...
    surface:{
      ...
      mediaDependency:'', interactionDependency:'', styleIntent:[]
    },
    acquire:{
      contentMode:'', freshnessSensitive:false, sources:[], assets:[], benchmarks:[], blockers:[]
    },
    direction:{
      dominantMedium:'', visualThesis:'', colorBehavior:'', typeBehavior:'',
      compositionBehavior:'', depthBehavior:'', motionBehavior:'', signatureMoment:''
    },
    gates:{understand:false,acquire:false,direct:false,build:false,review:false},
    review:{visualReviewed:false,notes:[]},
    qa:{status:null,reportPath:null,attempts:0},
...
if(cmd==='status'){
  ...
  console.log(JSON.stringify({active:s.active,phase:s.phase,target:s.target,gates:s.gates,qa:s.qa,review:s.review},null,2));
  process.exit(0);
}
...
if(cmd==='gate'){
  ...
  if(gate==='understand'){
    ...
    for(const k of ['dominantMedium','visualThesis','colorBehavior','typeBehavior','compositionBehavior','depthBehavior','motionBehavior']) req(s.direction?.[k],`direction.${k}`,errs);
  } else if(gate==='build'){
...
fail('unknown command. Use init|status|gate|finish');
