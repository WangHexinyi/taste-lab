# TASTE-OVERRIDE (delta-only v2.1: overrides only where skill defaults diverge)
- Aesthetic family: editorial/luxury; serif display justified (serif discipline override).
- Palette: morandi low-sat gray (HSL S 10-24%, L 46-64%), stage hue drawn per session;
  overrides "neutral + one high-contrast accent" default.
- Terracotta/clay hue h in [12,30] banned (overrides skill's Terracotta+Slate rotation).
- Dials: DESIGN_VARIANCE 8, MOTION_INTENSITY 8, VISUAL_DENSITY 5.
- All reveals replayable; one-shot banned.
- Never replace/skin native cursor.
- Output: single-file HTML (inline CSS/JS, CDN allowed), overrides React/Next default.
- CONTENT COVERAGE: sections decomposed 1:1 from brief; every named item
  (appearance/config/internal structure/materials) = own section with >=3 real
  content elements; missing item = fail. (skill does not enforce coverage; this wins.)
- PRODUCT-DETAIL MODE: this is a detail page, not a marketing landing;
  "cut ruthlessly" and <=25-word limits overridden; dense spec content allowed,
  but use skill's alternative spec UI (2-col card grid / grouped chunks),
  no hairline-per-row tables.
- COPY: every sentence must carry concrete info (model numbers/materials/figures);
  abstract pretentious phrasing = fail; use real public specs you know for this
  model; uncertain data labeled as sample.
- REVEAL HYSTERESIS: add .in-view at ratio >= 0.15; remove ONLY when ratio === 0
  (fully out of viewport) AND after 400ms grace; content still on screen must never fade out.
- NATIVE SCROLL IS SACRED: never preventDefault on wheel/touch; all motion driven by
  native scroll position; page must scroll normally even if every CDN script fails.
